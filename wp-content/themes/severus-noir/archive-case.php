		...
		<?php if ( $categories && ! is_wp_error( $categories ) ) : ?>
			<nav class="filter" aria-label="<?php esc_attr_e( 'Filter cases by category', 'severus-noir' ); ?>">
				<ul class="filter__list">
					<li>
						<a class="btn <?php echo $term ? 'btn--quiet' : 'btn--solid'; ?>" href="<?php echo esc_url( $archive ); ?>"<?php echo $term ? '' : ' aria-current="page"'; ?>><?php esc_html_e( 'All', 'severus-noir' ); ?></a>
					</li>
					<?php foreach ( $categories as $category ) : ?>
						<?php $current = $term && $term->term_id === $category->term_id; ?>
						<li>
							<a class="btn <?php echo $current ? 'btn--solid' : 'btn--quiet'; ?>" href="<?php echo esc_url( get_term_link( $category ) ); ?>"<?php echo $current ? ' aria-current="page"' : ''; ?>><?php echo esc_html( $category->name ); ?></a>
						</li>
					<?php endforeach; ?>
				</ul>
			</nav>
		...
		<?php if ( have_posts() ) : ?>
			...
			the_posts_pagination(
				array(
					'class'     => 'pager',
					'mid_size'  => 1,
					'prev_text' => __( 'Previous', 'severus-noir' ),
					'next_text' => __( 'Next', 'severus-noir' ),
				)
			);
			?>
		<?php else : ?>
